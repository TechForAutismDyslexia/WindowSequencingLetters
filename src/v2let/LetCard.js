...
const LetCard = ({ words }) => {
  ...
  const handleNextWord = () => {
    if (currentWordIndex + 1 < words.length) {
      ...
      setShowModal(true); // Show modal after all words are completed
    }
    setAnswer('');
  ...
  const handleModalClose = () => {
    ...
    setShowModal(false);
    sendTimeData(); // Send time data before redirecting
    navigate('/'); // Redirect to WordSelector component
  };
  ...
  const sendTimeData = async () => {
    try {
      if (startTime && endTime) {
        ...
        console.log('Time data sent to backend:', timeData);
      }
    } catch (error) {
  ...
  const handleEndGame = () => {
    ...
    sendTimeData(); // Send time data to backend
    setShowModal(true);
  };

  return (
    <div className="wsb">
      <h1 className="desc" style={{ marginTop: '1rem' }} onClick={() => pronounceWord('Look at the letters carefully. Match your letters to what you see')}>
      ...
      <div className="random-cards shadow" style={{ display: 'flex', flexWrap: 'wrap', marginTop: '1rem' }}>
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal-container">
            ...
            <div className="custom-modal-footer">
              <button className="custom-modal-button" onClick={handleModalClose}>Play Again</button>
            </div>
          </div>
        </div>
      )}
